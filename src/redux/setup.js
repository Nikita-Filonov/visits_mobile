...
import {SET_GROUP_TIMERS_COMMENT_SHOWN, SET_GROUP_TIMERS_SORT} from './GroupTimers/actionTypes';
...
export const setupReduxStore = async (store) => {
  ...
  const groupTimersSort = backup[0][1] || GROUP_TIMERS_INITIAL_STATE.groupTimersSort;
  const timersSort = backup[1][1] || TIMERS_INITIAL_STATE.timersSort;
  const timersCommentShown = JSON.parse(backup[2][1]) || TIMERS_INITIAL_STATE.commentShown;
  const groupTimersCommentShown = JSON.parse(backup[3][1]) || GROUP_TIMERS_INITIAL_STATE.commentShown;
  ...
  store.dispatch({type: SET_GROUP_TIMERS_COMMENT_SHOWN, payload: groupTimersCommentShown});
};
