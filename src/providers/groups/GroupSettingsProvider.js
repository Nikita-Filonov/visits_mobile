...
import {SET_GROUP_MONSTERS, SET_GROUP_SETTINGS} from '../../redux/Groups/actionTypes';
...
const GroupSettingsProvider = ({children, store}) => {
  ...
  const getGroupSettings = async (groupId: number, isLazy: boolean = false) => {
    ...
    (!error && !isLazy) && store.dispatch({type: SET_GROUP_SETTINGS, payload: json});
  ...
  const updateGroupSettings = async (groupId, payload) => {
    setRequest(true);
    const {json, error} = await patch(groupsApi + `${groupId}/settings/`, payload);
    !error && store.dispatch({type: SET_GROUP_SETTINGS, payload: json});
    setAlert(error ? json : successTemplate(GROUP_SETTINGS, SUPPORTED_ACTIONS.update));
    setRequest(false);
  };
...
const useGroupSettings = () => {
  ...
  if (event == null) {
    throw new Error('useGroupSettings() called outside of a GroupSettingsProvider?');
  }
  return event;
};
