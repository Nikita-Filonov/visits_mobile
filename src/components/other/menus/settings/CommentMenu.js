...
const CommentMenu = ({comment, setComment}) => {
  ...
  const onDelete = () => {
    onClose();
    setConfirmModal({
      action: async () => await updateSettings({comments: settings.comments.filter(c => c.id !== comment.id)}),
      content: {...t('settings.comments.deleteCommentModal'), confirmButton: t('common.delete')},
    });
  };
