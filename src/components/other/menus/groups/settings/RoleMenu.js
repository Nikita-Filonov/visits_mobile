...
const RoleMenu = ({role, group, setRole, setCreateRoleModal}) => {
  ...
  const onDelete = async () => {
    onClose();
    setConfirmModal({
      content: {...t('groups.settings.roles.deleteRoleModal'), confirmButton: t('common.delete')},
  ...
  return (
    <CustomMenu menu={menu} setMenu={setMenu}>
      <CustomMenuItem disabled={!isAllowed([ROLE.update])} onPress={onChange} title={t('common.change')}/>
      <CustomMenuItem disabled={!isAllowed([ROLE.delete])} onPress={onDelete} title={t('common.delete')}/>
    </CustomMenu>
  );
};
