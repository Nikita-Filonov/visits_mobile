...
import {setCreateMemberModal, setGroup, setMember} from '../../../../../redux/Groups/groupsActions';
...
const MemberMenu = ({member, group, setMember, setCreateMemberModal}) => {
  ...
  const onDelete = async () => {
    ...
    setConfirmModal({
      ...
      content: {...t('groups.settings.members.deleteMemberModal'), confirmButton: t('common.delete')},
    });
  };
  ...
  return (
    <CustomMenu menu={menu} setMenu={setMenu}>
      <CustomMenuItem disabled={!isAllowed([MEMBER.update])} onPress={onChange} title={t('common.change')}/>
      <CustomMenuItem
        disabled={!isAllowed([MEMBER.update])}
        onPress={onNotify}
        title={member?.notify ? t('groups.enableNotifications') : t('groups.disableNotifications')}
      />
      <CustomMenuItem disabled={!isAllowed([MEMBER.delete])} onPress={onDelete} title={t('common.delete')}/>
    </CustomMenu>
  );
};
