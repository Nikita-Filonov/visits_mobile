...
export const TelegramUserSyncStatus = () => {
  ...
  const isSynced = useMemo(() => Boolean(user?.telegram_id && user?.telegram_username), [user]);
  const isWaitingForSync = useMemo(() => Boolean(!user?.telegram_id && user?.telegram_username), [user]);
  ...
  const onPong = async () => {
    ...
    (!error && json?.telegram_id) && setUser({...user, telegram_id: json?.telegram_id});
  };
