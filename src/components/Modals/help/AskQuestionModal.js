...
export const AskQuestionModal = ({modal, setModal}) => {
  ...
  const onSend = async () => {
    setRequest(true);
    const {error, json} = await post('api/v1/questions/', {
      subject,
      question,
      feedback,
    });
    setAlert(
      error
        ? json
        : {message: t('help.askQuestionModal.alert'), level: 'success'},
    );
    onClose();
    setSubject('');
    setQuestion('');
  ...
  const isDisabled = useMemo(
    () =>
      question.length === 0 || feedback.length === 0 || subject.length === 0,
    [question, feedback, subject],
  );
    ...
    <FullScreenModal modal={modal} setModal={setModal}>
      <FullScreenModalConfirmLayout
        ...
        <CustomText style={comp.smallText}>
          {t('help.askQuestionModal.description')}
        </CustomText>
