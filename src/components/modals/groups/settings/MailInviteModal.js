...
const MailInviteModal = ({modal, setModal, group}) => {
  ...
  const sendInvitations = async () => mailInvitation(group.id, selectedEmails).then(() => onClose());
  ...
  const removeEmail = (email) => setSelectedEmails(selectedEmails.filter(e => e !== email));

  const error = useMemo(() => selectedEmails.some(email => !validateEmail(email)), [selectedEmails]);
    ...
    <FullScreenModal modal={modal} onClose={onClose}>
      ...
      >
        <TextField
          ...
          label={t('login.email')}
          placeholder={t('groups.settings.members.mailInviteModal.emailInputPlaceholder')}
          right={email.length > 0 &&
          <TextInput.Icon name={() => <Touchable action={appendEmail} name={'check'} type={'feather'}/>}/>}
        />
        <FlatList
          style={comp.input}
          data={selectedEmails}
          ...
          renderItem={({item}) => <Mail item={item} removeEmail={removeEmail}/>}
