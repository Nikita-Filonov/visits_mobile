...
import {setCreateMemberModal, setMember} from '../../../../redux/Groups/groupsActions';
...
const InviteMemberModal = ({createMemberModal, setCreateMemberModal, group, member, setMember}) => {
  ...
  const onInvite = async () => (member?.editMode
    ? updateMember(group.id, member.id, {role: member?.role?.id})
    : inviteMember(group.id, {role: member?.role?.id, username: member?.username}))
    .then(() => onClose());

  const getRoleName = (roleId) => group?.roles?.find(r => r.id === roleId)?.name;
  const onSelectRole = () => setSelectRoleModal(true);
  const disabled = useMemo(() => !member?.role?.id || member?.username?.length === 0, [member]);
  ...
  return (
    <FullScreenModal modal={createMemberModal} onClose={onClose}>
      <FullScreenModalConfirmLayout
        loading={request}
        onClose={onClose}
        title={t(`groups.settings.members.${member.editMode ? 'editMember' : 'addMember'}`)}
        onConfirm={onInvite}
        disabled={disabled}
      >
        <ScrollView>
          {!member.editMode && <TextField
          ...
          <TouchableOpacity onPress={onSelectRole}>
            <TextField
              ...
              right={
                <TextInput.Icon
                  name={() =>
                    <Touchable action={onSelectRole} name={'person-add-alt'} type={'material'}/>
                  }
                />
              }
            />
        ...
        <SelectMemberRoleModal modal={selectRoleModal} setModal={setSelectRoleModal}/>
      </FullScreenModalConfirmLayout>
    </FullScreenModal>
  );
};
